...
class TrainerSegmentMPMorph:
    ...
    def initialize_optimizer_and_scheduler(self):
        ...
        self.optimizer = optim.Adam([{'params': self.model.parameters()},
                                     {'params': self.voxelmorph_model.parameters(), 'lr': learning_rate*10}],
    ...
    def _forward_pass(self, image, label, kplabel, atlas, atlas_label, **kwargs):
        ...
        y_source, pos_flow = self.voxelmorph_model(image, label.sum(dim=1, keepdim=True), atlas, atlas_label.sum(dim=1, keepdim=True), True)
        ...
        if self.num_epoch > 10:
            loss = lossseg + lossskp + loss_morph + loss_warp_seg
        else:
            loss = lossseg + lossskp + loss_morph

        loss_dict = {'SegLoss': lossseg.item(),
                     'KpLoss': lossskp.item(),
